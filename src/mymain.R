...
update_forecast <- function(test_month, dept_preds, dept, num_model) {
  ...
  if (num_model == 1) {
    ...
    test_month$Weekly_Pred4[pred.d.idx] <- pred.d$value
  }
...
# when test left-join test_month, Weekly_Pred would become Weekly_Pred.x 
# and Weekly_Pred.y because both test and test_month have same column name
...
mypredict <- function() {
  ...
  for (dept in test_depts) {
    ...
    tslm_naive <- tslm_model(train_dept_ts, test_dept_ts)
    arima <- arima_model(train_dept_ts, test_dept_ts)
    test_month <- update_forecast(test_month, f_naive, dept, 1)
    test_month <- update_forecast(test_month, tslm_naive, dept, 2)
    test_month <- update_forecast(test_month, arima, dept, 3)
